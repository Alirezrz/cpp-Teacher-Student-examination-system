...
student_score Exam::Execution(ReportCard& sheet,string id){
    string name= Find_name_of_person(id);
    ...
    for (int i = 0; i < Questions.size(); i++) {
        ...
        sheet.Correct_Answers.push_back(Questions.at(i).Answer);
            ...
            for(auto &teacher : *teacher_list){
                if(teacher.getID() == this->designer_id){
                    Student_answer answer;
                    answer.student_id = id;
                    answer.Exam_code=this->Code;
                    answer.student_answer= ans;
                    if(ans==Questions.at(i).Answer){
                        answer.sent_score=Questions.at(i).Positive_effect;
                    }
                    else{
                        answer.sent_score=Questions.at(i).Negative_effect;
                    }
                    answer.Question_body= Questions.at(i).question_body;
                    answer.Question_correct_answer=Questions.at(i).Answer;

                    teacher.correction_list.push_back(answer);
...
Exam json_to_Exam(json j_exam){
    ...
    new_exam.designer_id=j_exam["designer_id"];
...
void Exam::display_grades_to_teacher() {
    ...
    cout << string(40, '-') << "\n";
    cout << left << setw(25) << "Highest Grade:" 
         << "  " << CYAN << max_name << " (" << max_scr << ")" << RESET << endl;
    cout << UNDERLINE << left << setw(25) << "Class Average:" 
         << "  " << fixed << setprecision(2) << average << RESET << "\n\n";
