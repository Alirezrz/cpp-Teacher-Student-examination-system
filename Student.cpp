...
void Student::attend_exam(){
    ...
    for(int i=0;i<created_Exams_list->size();i++){
        if(code == created_Exams_list->at(i).get_code()){
            student_score score = created_Exams_list->at(i).Execution(this->Scores_table,this -> ID);
...
int Student::Rank_Computer() {
    ...
    auto it = find(averages.begin(), averages.end(), this->Average_score());
    if (it != averages.end()) {
        return distance(averages.begin(), it) + 1;
    }
